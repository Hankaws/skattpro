...
export default async function handler(request) {
  ...
  if (!webhookSecret) {
    return new Response(JSON.stringify({ error: 'Server misconfigured: missing STRIPE_WEBHOOK_SECRET' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
  ...
  try {
    ...
    console.error('webhook_construct_failed', error);
  ...
  if (!email) {
    return new Response(JSON.stringify({ error: 'Missing customer email in checkout session' }), {
      status: 400,
      headers: { 'Content-Type': 'application/json' },
    });
  }
